...
import com.neighborsystem.mapper.GuessMapper;
...
public class GuessServiceImpl implements IGuessService {
	...
	@Transactional(propagation=Propagation.REQUIRED,readOnly=true)
	@Override
	public PageInfo<Guess> findAllFavoriteByUserId(Integer userId,Integer page,Integer limit) {
		PageHelper.startPage(page, limit);
		List<Guess> list = guessMapper.findAllFavoriteByUserId(userId);
		PageInfo<Guess> info=new PageInfo<Guess>(list);
		return info;
	}
	@Transactional(propagation=Propagation.REQUIRED,rollbackFor=Exception.class)
	@Override
